...
class EmbeddingInjectionModel:
    ...
    def get_input_embeddings(self, input_ids, pixel_values=None, **_kwargs):
        if pixel_values is None:
            # Some models reset position state in this path; prompt caching must
            # restore it from the bundle before the first LM forward.
            self.language_model._rope_deltas = "WRONG"
        seq_len = int(input_ids.shape[1])
